...
{
    assert(node1->type == NECRO_AST_TYPE_ATTRIBUTE);
    assert(node2->type == NECRO_AST_TYPE_ATTRIBUTE);
    necro_parse_ast_assert_eq_go(ast1, node1->attribute.attributed_type, ast2, node2->attribute.attributed_type);
    assert(node1->attribute.type == node2->attribute.type);
}
