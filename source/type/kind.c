...
inline void necro_kind_unify_fun(NecroInfer* infer, NecroTypeKind* kind1, NecroTypeKind* kind2, NecroScope* scope, NecroType* macro_type, const char* error_preamble)
{
    ...
    case NECRO_TYPE_APP:  necro_infer_error(infer, error_preamble, macro_type, "Kind mismatch: Attempting to unify (->) with TypeApp."); return;
    case NECRO_TYPE_CON:  necro_infer_error(infer, error_preamble, macro_type, "Kind mismatch: Attempting to unify (->) with TypeCon (%s)", necro_intern_get_string(infer->intern, kind2->con.con.symbol)); return;
...
inline void necro_kind_unify_con(NecroInfer* infer, NecroTypeKind* kind1, NecroTypeKind* kind2, NecroScope* scope, NecroType* macro_type, const char* error_preamble)
{
    ...
    {
    case NECRO_TYPE_VAR:
        // necro_unify_kinds(infer, kind2, &type2->kind, &type1->kind, macro_type, error_preamble);
        if (kind2->var.is_rigid)
            necro_rigid_kind_variable_error(infer, kind2->var.var, kind1, macro_type, error_preamble);
        else if (necro_occurs(infer, kind2, kind1, macro_type, error_preamble))
            return;
    ...
    case NECRO_TYPE_CON:
        ...
        else
        {
            ...
            while (kind1 != NULL && kind2 != NULL)
            {
                ...
                {
                    necro_infer_error(infer, error_preamble, kind1, "Mismatched arities, Kind1: %s, Kind2: %s", necro_intern_get_string(infer->intern, kind1->con.con.symbol), necro_intern_get_string(infer->intern, kind2->con.con.symbol));
                    return;
                }
                ...
                // necro_kind_unify_kinds(infer, type2, &type2->list.item->kind, &type1->list.item->kind, macro_type, error_preamble);
    ...
    case NECRO_TYPE_FUN:  necro_infer_error(infer, error_preamble, macro_type, "Attempting to unify KindCon (%s) with (->).", necro_intern_get_string(infer->intern, kind1->con.con.symbol)); return;
...
void necro_kind_unify(NecroInfer* infer, NecroTypeKind* kind1, NecroTypeKind* kind2, NecroScope* scope, NecroType* macro_type, const char* error_preamble)
{
    ...
    assert(kind1 != NULL);
    assert(kind2 != NULL);
    assert(kind1->kind == NULL);
    assert(kind2->kind == NULL);
