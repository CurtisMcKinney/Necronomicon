...
// TODO: Make this a machine_def not a fn_def!
void necro_declare_apply_fn(NecroMachineProgram* program, size_t apply_arity)
{
    ...
    // TODO: Add machine_def struct ptr as argument!
    ...
    // Need to retain data_id somewhere for gc!
    // Special data_id id for _apply machines since they involve dynamic data_ids
...
{
    assert(program != NULL);
    for (size_t i = 0; i < program->apply_defs.length; ++i)
    {
        necro_declare_apply_fn(program, program->apply_defs.data[i].apply_arity + 1);
    }
}
