...
NecroParser necro_parser_create(NecroLexToken* tokens, size_t num_tokens, NecroIntern* intern)
{
    return (NecroParser)
    {
        .current_token          = 0,
...
void necro_parse_ast_print_go(NecroParseAstArena* ast, NecroParseAst* ast_node, NecroIntern* intern, uint32_t depth)
{
    assert(ast != NULL);
    ...
    case NECRO_AST_BIN_OP:
        ...
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->bin_op.lhs), intern, depth + 1);
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->bin_op.rhs), intern, depth + 1);
        break;
    ...
    case NECRO_AST_IF_THEN_ELSE:
        puts("(If then else)");
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->if_then_else.if_expr), intern, depth + 1);
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->if_then_else.then_expr), intern, depth + 1);
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->if_then_else.else_expr), intern, depth + 1);
        break;

    case NECRO_AST_TOP_DECL:
        puts("(Top Declaration)");
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->top_declaration.declaration), intern, depth + 1);
        if (ast_node->top_declaration.next_top_decl != null_local_ptr)
        {
            necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->top_declaration.next_top_decl), intern, depth);
        }
        break;

    case NECRO_AST_DECL:
        puts("(Declaration)");
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->declaration.declaration_impl), intern, depth + 1);
        if (ast_node->declaration.next_declaration != null_local_ptr)
        {
            necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->declaration.next_declaration), intern, depth);
        }
        break;
    ...
    case NECRO_AST_SIMPLE_ASSIGNMENT:
        ...
        if (ast_node->simple_assignment.initializer != null_local_ptr)
        {
            ...
            printf("<\n");
            necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->simple_assignment.initializer), intern, depth);
            print_white_space(depth * 2);
            printf(">\n");
        }
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->simple_assignment.rhs), intern, depth + 1);
        break;

    case NECRO_AST_RIGHT_HAND_SIDE:
        puts("(Right Hand Side)");
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->right_hand_side.expression), intern, depth + 1);
        if (ast_node->right_hand_side.declarations != null_local_ptr)
        {
            ...
            necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->right_hand_side.declarations), intern, depth + 3);
        }
        break;

    case NECRO_AST_LET_EXPRESSION:
        puts("(Let)");
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->let_expression.declarations), intern, depth + 1);
        if (ast_node->right_hand_side.declarations != null_local_ptr)
        {
            for (uint32_t i = 0;  i < depth; ++i)
            {
            ...
            necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->let_expression.expression), intern, depth + 3);
        }
        break;
    ...
    case NECRO_AST_FUNCTION_EXPRESSION:
        puts("(fexp)");
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->fexpression.aexp), intern, depth + 1);
        if (ast_node->fexpression.next_fexpression != null_local_ptr)
        {
            necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->fexpression.next_fexpression), intern, depth + 1);
        }
        break;
    ...
    case NECRO_AST_APATS:
        puts("(Apat)");
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->apats.apat), intern, depth + 1);
        if (ast_node->apats.next_apat != null_local_ptr)
        {
            necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->apats.next_apat), intern, depth);
        }
        break;
    ...
    case NECRO_AST_APATS_ASSIGNMENT:
        ...
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->apats_assignment.apats), intern, depth + 1);
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->apats_assignment.rhs), intern, depth + 1);
        break;

    case NECRO_AST_PAT_ASSIGNMENT:
        puts("(Pat Assignment)");
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->pat_assignment.pat), intern, depth + 1);
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->pat_assignment.rhs), intern, depth + 1);
        break;

    case NECRO_AST_LAMBDA:
        puts("\\(lambda)");
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->lambda.apats), intern, depth + 1);
        for (uint32_t i = 0;  i < (depth + 1); ++i)
        {
            printf(STRING_TAB);
        }
        puts("->");
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->lambda.expression), intern, depth + 2);
        break;

    case NECRO_AST_DO:
        puts("(do)");
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->do_statement.statement_list), intern, depth + 1);
        break;

    case NECRO_AST_PAT_EXPRESSION:
        puts("(pat)");
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->pattern_expression.expressions), intern, depth + 1);
        break;

    case NECRO_AST_EXPRESSION_LIST:
        puts("([])");
        if (ast_node->expression_list.expressions != null_local_ptr)
            necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->expression_list.expressions), intern, depth + 1);
        break;

    case NECRO_AST_EXPRESSION_ARRAY:
        puts("({})");
        if (ast_node->expression_array.expressions != null_local_ptr)
            necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->expression_array.expressions), intern, depth + 1);
        break;
    ...
    case NECRO_AST_LIST_NODE:
        ...
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->list.item), intern, depth);
        if (ast_node->list.next_item != null_local_ptr)
        {
            necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->list.next_item), intern, depth);
        }
        break;

    case NECRO_BIND_ASSIGNMENT:
        printf("(Bind: %s)\n", necro_intern_get_string(intern, ast_node->bind_assignment.variable_name));
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->bind_assignment.expression), intern, depth + 1);
        break;

    case NECRO_PAT_BIND_ASSIGNMENT:
        printf("(Pat Bind)\n");
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->pat_bind_assignment.pat), intern, depth + 1);
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->pat_bind_assignment.expression), intern, depth + 1);
        break;
    ...
    case NECRO_AST_ARITHMETIC_SEQUENCE:
        {
            ...
            case NECRO_ARITHMETIC_ENUM_FROM:
                puts("(EnumFrom)");
                necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->arithmetic_sequence.from), intern, depth + 1);
                break;
            case NECRO_ARITHMETIC_ENUM_FROM_TO:
                puts("(EnumFromTo)");
                necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->arithmetic_sequence.from), intern, depth + 1);
                necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->arithmetic_sequence.to), intern, depth + 1);
                break;
            case NECRO_ARITHMETIC_ENUM_FROM_THEN_TO:
                puts("(EnumFromThenTo)");
                necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->arithmetic_sequence.from), intern, depth + 1);
                necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->arithmetic_sequence.then), intern, depth + 1);
                necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->arithmetic_sequence.to), intern, depth + 1);
                break;
            default:
                assert(false);
    ...
    case NECRO_AST_CASE:
        puts("case");
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->case_expression.expression), intern, depth + 1);
        printf("\r"); // clear current line
        for (uint32_t i = 0;  i < depth; ++i) printf(STRING_TAB);
        puts("of");
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->case_expression.alternatives), intern, depth + 1);
        break;

    case NECRO_AST_CASE_ALTERNATIVE:
        printf("\r"); // clear current line
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->case_alternative.pat), intern, depth + 0);
        printf("\r"); // clear current line
        for (uint32_t i = 0;  i < depth; ++i) printf(STRING_TAB);
        puts("->");
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->case_alternative.body), intern, depth + 1);
        break;
    ...
    case NECRO_AST_DATA_DECLARATION:
        puts("(data)");
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->data_declaration.simpletype), intern, depth + 1);
        for (uint32_t i = 0;  i < depth; ++i) printf(STRING_TAB);
        puts(" = ");
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->data_declaration.constructor_list), intern, depth + 1);
        break;

    case NECRO_AST_SIMPLE_TYPE:
        puts("(simple type)");
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->simple_type.type_con), intern, depth + 1);
        if (ast_node->simple_type.type_var_list != null_local_ptr)
            necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->simple_type.type_var_list), intern, depth + 2);
        break;

    case NECRO_AST_CONSTRUCTOR:
        puts("(constructor)");
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->constructor.conid), intern, depth + 1);
        if (ast_node->constructor.arg_list != null_local_ptr)
            necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->constructor.arg_list), intern, depth + 1);
        break;

    case NECRO_AST_TYPE_APP:
        puts("(type app)");
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->type_app.ty), intern, depth + 1);
        if (ast_node->type_app.next_ty != null_local_ptr)
            necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->type_app.next_ty), intern, depth + 1);
        break;

    case NECRO_AST_BIN_OP_SYM:
        printf("(%s)\n", necro_intern_get_string(intern, necro_parse_ast_get_node(ast, ast_node->bin_op_sym.op)->conid.symbol));
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->bin_op_sym.left), intern, depth + 1);
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->bin_op_sym.right), intern, depth + 1);
        break;

    case NECRO_AST_OP_LEFT_SECTION:
        printf("(LeftSection %s)\n", necro_bin_op_name(ast_node->op_left_section.type));
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->op_left_section.left), intern, depth + 1);
        break;

    case NECRO_AST_OP_RIGHT_SECTION:
        printf("(%s RightSection)\n", necro_bin_op_name(ast_node->op_right_section.type));
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->op_right_section.right), intern, depth + 1);
        break;

    case NECRO_AST_TYPE_SIGNATURE:
        printf("\r");
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->type_signature.var), intern, depth + 0);
        for (uint32_t i = 0;  i < depth + 1; ++i) printf(STRING_TAB);
        puts("::");
        if (ast_node->type_signature.context != null_local_ptr)
        {
            necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->type_signature.context), intern, depth + 1);
            for (uint32_t i = 0;  i < depth + 2; ++i) printf(STRING_TAB);
            puts("=>");
        }
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->type_signature.type), intern, depth + 1);
        break;

    case NECRO_AST_TYPE_CLASS_CONTEXT:
        printf("\r");
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->type_class_context.conid), intern, depth + 0);
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->type_class_context.varid), intern, depth + 0);
        break;
    ...
    case NECRO_AST_TYPE_CLASS_DECLARATION:
        ...
        {
            necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->type_class_declaration.context), intern, depth + 1);
            for (uint32_t i = 0;  i < depth + 2; ++i) printf(STRING_TAB);
            puts("=>");
        }
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->type_class_declaration.tycls), intern, depth + 1);
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->type_class_declaration.tyvar), intern, depth + 1);
        if (ast_node->type_class_declaration.declarations != null_local_ptr)
        {
            for (uint32_t i = 0; i < depth; ++i) printf(STRING_TAB);
            puts("where");
            necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->type_class_declaration.declarations), intern, depth + 2);
        }
        break;
    ...
    case NECRO_AST_TYPE_CLASS_INSTANCE:
        ...
        {
            necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->type_class_instance.context), intern, depth + 1);
            for (uint32_t i = 0;  i < depth + 2; ++i) printf(STRING_TAB);
            puts("=>");
        }
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->type_class_instance.qtycls), intern, depth + 1);
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->type_class_instance.inst), intern, depth + 1);
        if (ast_node->type_class_declaration.declarations != null_local_ptr)
        {
            for (uint32_t i = 0; i < depth; ++i) printf(STRING_TAB);
            puts("where");
    ...
    case NECRO_AST_FUNCTION_TYPE:
        puts("(");
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->function_type.type), intern, depth + 1);
        for (uint32_t i = 0;  i < depth + 0; ++i) printf(STRING_TAB);
        puts("->");
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, ast_node->function_type.next_on_arrow), intern, depth + 1);
        for (uint32_t i = 0;  i < depth + 0; ++i) printf(STRING_TAB);
        puts(")");
        break;
...
void necro_parse_ast_print(NecroParseAstArena* ast, NecroIntern* intern, NecroParseAstLocalPtr root_node_ptr)
{
    ...
    {
        necro_parse_ast_print_go(ast, necro_parse_ast_get_node(ast, root_node_ptr), intern, 0);
    }
}
...
NecroResult(void) necro_parse(NecroCompileInfo info, NecroIntern* intern, NecroLexTokenVector* tokens, NecroParseAstArena* out_ast)
{
    ...
    *out_ast = parser.ast;
    ...
    {
        necro_parse_ast_print(out_ast, intern, local_ptr);
    }
    necro_parser_destroy(&parser);
    return ok_void();
}

inline bool necro_is_parse_result_non_error_null(NecroResult(NecroParseAstLocalPtr) result)
{
...
NecroResult(NecroParseAstLocalPtr) necro_parse_top_declarations(NecroParser* parser)
{
    ...
    else
    {
        ...
        {
            necro_parse_consume_token(parser);
            next_top_decl = necro_try(NecroParseAstLocalPtr, necro_parse_top_declarations(parser));
        }
...
NecroResult(NecroParseAstLocalPtr) necro_parse_declaration(NecroParser* parser)
{
    ...
    {
        declaration_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_type_signature(parser, NECRO_SIG_DECLARATION));
    }

    if (declaration_local_ptr == null_local_ptr)
    {
        declaration_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_simple_assignment(parser));
    }

    if (declaration_local_ptr == null_local_ptr)
    {
        declaration_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_apats_assignment(parser));
    }

    if (declaration_local_ptr == null_local_ptr)
    {
        declaration_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_pat_assignment(parser));
    }
...
NecroResult(NecroParseAstLocalPtr) necro_parse_declarations_list(NecroParser* parser)
{
    ...
    else
    {
        ...
        {
            necro_parse_consume_token(parser);
            next_decl = necro_try(NecroParseAstLocalPtr, necro_parse_declarations_list(parser));
        }
        // Finish
...
NecroResult(NecroParseAstLocalPtr) necro_parse_declarations(NecroParser* parser)
{
    ...
    // Declaration list
    NecroParseAstLocalPtr declarations_list_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_declarations_list(parser));
...
NecroResult(NecroParseAstLocalPtr) necro_parse_simple_assignment(NecroParser* parser)
{
    ...
    if (necro_parse_peek_token_type(parser) == NECRO_INITIALIZER_TOKEN)
    {
        ...
        initializer = necro_try(NecroParseAstLocalPtr, necro_parse_initializer(parser));
    ...
    // Rhs
    NecroLexToken*        look_ahead_token = necro_parse_peek_token(parser);
    NecroParseAstLocalPtr rhs_local_ptr    = necro_try(NecroParseAstLocalPtr, necro_parse_right_hand_side(parser));
    ...
    // Finish
    NecroParseAstLocalPtr assignment_local_ptr = necro_parse_ast_create_simple_assignment(&parser->ast.arena, source_loc, necro_parse_peek_token(parser)->source_loc, variable_name_token->symbol, rhs_local_ptr, initializer);
...
NecroResult(NecroParseAstLocalPtr) necro_parse_apats_assignment(NecroParser* parser)
{
    ...
    // Apats
    NecroParseAstLocalPtr apats_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_apats(parser));
    ...
    // Finish
    NecroParseAstLocalPtr assignment_local_ptr = necro_parse_ast_create_apats_assignment(&parser->ast.arena, source_loc, necro_parse_peek_token(parser)->source_loc, variable_name_token->symbol, apats_local_ptr, rhs_local_ptr);
    return ok_NecroParseAstLocalPtr(assignment_local_ptr);
}
...
NecroResult(NecroParseAstLocalPtr) necro_parse_pat_assignment(NecroParser* parser)
{
    ...
    NecroParseAstLocalPtr pat_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_pat(parser));
    parser->parsing_pat_assignment  = false;
...
NecroResult(NecroParseAstLocalPtr) necro_parse_right_hand_side(NecroParser* parser)
{
    ...
    NecroSourceLoc        source_loc           = necro_parse_peek_token(parser)->source_loc;
    NecroParserSnapshot   snapshot             = necro_parse_snapshot(parser);
    NecroParseAstLocalPtr expression_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_expression(parser));
    ...
    {
        necro_parse_consume_token(parser);
        declarations_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_declarations(parser));
...
NecroResult(NecroParseAstLocalPtr) parse_let_expression(NecroParser* parser)
{
    ...
    // Declarations
    NecroParseAstLocalPtr declarations_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_declarations(parser));
    if (declarations_local_ptr == null_local_ptr)
    {
...
NecroResult(NecroParseAstLocalPtr) necro_parse_expression_go(NecroParser* parser, NECRO_BINARY_LOOK_AHEAD_TYPE look_ahead_binary)
{
    ...
    if (local_ptr == null_local_ptr)
    {
        local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_l_expression(parser));
    }

    // BinOp
    if (look_ahead_binary == NECRO_BINARY_LOOK_AHEAD && local_ptr != null_local_ptr)
    {
        NecroParseAstLocalPtr bin_op_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_binary_operation(parser, local_ptr));
...
NecroParseAstLocalPtr necro_parse_variable(NecroParser* parser, NECRO_VAR_TYPE var_type)
{
    ...
    {
        NecroParseAstLocalPtr varid_local_ptr = necro_parse_ast_create_var(&parser->ast.arena, source_loc, necro_parse_peek_token(parser)->source_loc, variable_token->symbol, var_type, null_local_ptr);
...
NecroResult(NecroParseAstLocalPtr) necro_parse_application_expression(NecroParser* parser)
{
    ...
    if (local_ptr == null_local_ptr)
    {
        local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_gcon(parser, NECRO_CON_VAR));
    }
    ...
    if (local_ptr == null_local_ptr)
    {
        local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_op_left_section(parser));
    }

    if (local_ptr == null_local_ptr)
    {
        local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_op_right_section(parser));
    }

    if (local_ptr == null_local_ptr)
    {
        local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_parenthetical_expression(parser));
    }

    if (local_ptr == null_local_ptr)
    {
        local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_tuple(parser));
    }

    if (local_ptr == null_local_ptr)
    {
        local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_expression_list(parser));
    }

    if (local_ptr == null_local_ptr)
    {
        local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_expression_array(parser));
    }

    if (local_ptr == null_local_ptr)
    {
        local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_arithmetic_sequence(parser));
    }
...
NecroResult(NecroParseAstLocalPtr) necro_parse_function_expression_go(NecroParser* parser, NecroParseAstLocalPtr left)
{
    ...
    NecroSourceLoc        source_loc  = necro_parse_peek_token(parser)->source_loc;
    NecroParserSnapshot   snapshot    = necro_parse_snapshot(parser);
    NecroParseAstLocalPtr aexpression = necro_try(NecroParseAstLocalPtr, necro_parse_application_expression(parser));
...
NecroResult(NecroParseAstLocalPtr) necro_parse_function_expression(NecroParser* parser)
{
    ...
    // aexpression
    NecroParserSnapshot   snapshot    = necro_parse_snapshot(parser);
    NecroParseAstLocalPtr expr        = necro_try(NecroParseAstLocalPtr, necro_parse_application_expression(parser));
    NecroParseAstLocalPtr aexpression = necro_try(NecroParseAstLocalPtr, necro_parse_function_expression_go(parser, expr));
    if (aexpression == null_local_ptr)
    {
        necro_parse_restore(parser, snapshot);
        return ok_NecroParseAstLocalPtr(null_local_ptr);
...
NecroResult(NecroParseAstLocalPtr) necro_parse_const_acon(NecroParser* parser)
{
    ...
    if (local_ptr == null_local_ptr)
    {
        local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_parenthetical_const_con(parser));
    }

    if (local_ptr == null_local_ptr)
    {
        local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_const_tuple(parser));
    }

    if (local_ptr == null_local_ptr)
    {
        local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_const_list(parser));
    }
...
NecroResult(NecroParseAstLocalPtr) necro_parse_const_con_go(NecroParser* parser, NecroParseAstLocalPtr left)
{
    ...
    NecroSourceLoc        source_loc  = necro_parse_peek_token(parser)->source_loc;
    NecroParserSnapshot   snapshot    = necro_parse_snapshot(parser);
    NecroParseAstLocalPtr aexpression = necro_try(NecroParseAstLocalPtr, necro_parse_const_acon(parser));
    if (aexpression == null_local_ptr)
    {
        necro_parse_restore(parser, snapshot);
        return ok_NecroParseAstLocalPtr(left);
...
NecroResult(NecroParseAstLocalPtr) necro_parse_const_con(NecroParser* parser)
{
    if (necro_parse_peek_token_type(parser) == NECRO_LEX_END_OF_STREAM)
        return ok_NecroParseAstLocalPtr(null_local_ptr);
    NecroParserSnapshot   snapshot    = necro_parse_snapshot(parser);
    NecroParseAstLocalPtr con         = necro_try(NecroParseAstLocalPtr, necro_parse_const_acon(parser));
    NecroParseAstLocalPtr aexpression = necro_try(NecroParseAstLocalPtr, necro_parse_const_con_go(parser, con));
    if (aexpression == null_local_ptr)
    {
        necro_parse_restore(parser, snapshot);
        return ok_NecroParseAstLocalPtr(null_local_ptr);
...
NecroResult(NecroParseAstLocalPtr) necro_parse_const_tuple(NecroParser* parser)
{
    ...
    // Expressions
    NecroParseAstLocalPtr statement_list_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_list(parser, NECRO_LEX_COMMA, necro_parse_const_con));
...
NecroResult(NecroParseAstLocalPtr) necro_parse_const_list(NecroParser* parser)
{
    ...
    // Expressions
    NecroParseAstLocalPtr list_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_list(parser, NECRO_LEX_COMMA, necro_parse_const_con));
...
NecroResult(NecroParseAstLocalPtr) necro_parse_apat(NecroParser* parser)
{
    ...
    if (local_ptr == null_local_ptr)
    {
        local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_wildcard(parser));
    }
    ...
    if (local_ptr == null_local_ptr)
    {
        ...
        {
            necro_parse_consume_token(parser);
            local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_pat(parser));
            if (local_ptr != null_local_ptr)
            {
                if (necro_parse_peek_token_type(parser) == NECRO_LEX_RIGHT_PAREN)
                {
    ...
    if (local_ptr == null_local_ptr)
    {
        local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_tuple_pattern(parser));
    }

    // list pattern [pat1, ... , patk]
    if (local_ptr == null_local_ptr)
    {
        local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_list_pat(parser));
    }
...
NecroResult(NecroParseAstLocalPtr) necro_parse_apats(NecroParser* parser)
{
    ...
    NecroParseAstLocalPtr apat_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_apat(parser));
    if (apat_local_ptr != null_local_ptr)
    {
        NecroParseAstLocalPtr next_apat_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_apats(parser));
...
NecroResult(NecroParseAstLocalPtr) necro_parse_parenthetical_expression(NecroParser* parser)
{
    ...
    // Expression
    look_ahead_token                = necro_parse_peek_token(parser);
    NecroParseAstLocalPtr local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_expression(parser));
    if (local_ptr == null_local_ptr)
    {
...
NecroResult(NecroParseAstLocalPtr) necro_parse_binary_expression(NecroParser* parser, NecroParseAstLocalPtr lhs_local_ptr, int min_precedence)
{
    ...
    while (true)
    {
        ...
        rhs_local_ptr                                       = necro_try(NecroParseAstLocalPtr, necro_parse_expression_go(parser, NECRO_NO_BINARY_LOOK_AHEAD));
        NecroLexToken*           look_ahead_token           = necro_parse_peek_token(parser);
               ...
               look_ahead_bin_op_behavior.precedence >= next_min_precedence)
        {
            ...
            rhs_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_binary_expression(parser, rhs_local_ptr, next_min_precedence));
...
NecroResult(NecroParseAstLocalPtr) necro_parse_op_left_section(NecroParser* parser)
{
    ...
    // Left expression
    NecroParseAstLocalPtr left = necro_try(NecroParseAstLocalPtr, necro_parse_expression_go(parser, NECRO_NO_BINARY_LOOK_AHEAD));
...
NecroResult(NecroParseAstLocalPtr) necro_parse_op_right_section(NecroParser* parser)
{
    ...
    // Right expression
    const NecroParseAstLocalPtr right = necro_try(NecroParseAstLocalPtr, necro_parse_expression_go(parser, NECRO_NO_BINARY_LOOK_AHEAD));
...
NecroResult(NecroParseAstLocalPtr) necro_parse_l_expression(NecroParser* parser)
{
    ...
    if (local_ptr == null_local_ptr)
    {
        local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_pattern_expression(parser));
    }

    if (local_ptr == null_local_ptr)
    {
        local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_do(parser));
    }

    if (local_ptr == null_local_ptr)
    {
        local_ptr = necro_try(NecroParseAstLocalPtr, parse_let_expression(parser));
    }

    if (local_ptr == null_local_ptr)
    {
        local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_if_then_else_expression(parser));
    }

    if (local_ptr == null_local_ptr)
    {
        local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_function_expression(parser));
    }

    if (local_ptr == null_local_ptr)
    {
        local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_lambda(parser));
    }

    if (local_ptr == null_local_ptr)
    {
        local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_case(parser));
    }
...
NecroResult(NecroParseAstLocalPtr) necro_parse_if_then_else_expression(NecroParser* parser)
{
    ...
    // if expression
    NecroLexToken*        look_ahead_token = necro_parse_peek_token(parser);
    NecroParseAstLocalPtr if_local_ptr     = necro_try(NecroParseAstLocalPtr, necro_parse_expression(parser));
    ...
    // then expression
    look_ahead_token                     = necro_parse_peek_token(parser);
    NecroParseAstLocalPtr then_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_expression(parser));
    ...
    // else expression
    look_ahead_token                     = necro_parse_peek_token(parser);
    NecroParseAstLocalPtr else_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_expression(parser));
...
NecroResult(NecroParseAstLocalPtr) necro_parse_list(NecroParser* parser, NECRO_LEX_TOKEN_TYPE token_divider, NecroParseFunc parse_func)
{
    ...
    NecroSourceLoc        source_loc     = necro_parse_peek_token(parser)->source_loc;
    NecroParserSnapshot   snapshot       = necro_parse_snapshot(parser);
    NecroParseAstLocalPtr item_local_ptr = necro_try(NecroParseAstLocalPtr, parse_func(parser));
    ...
    if (necro_parse_peek_token_type(parser) == token_divider)
    {
        ...
        next_item_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_list(parser, token_divider, parse_func));
    }
...
NecroResult(NecroParseAstLocalPtr) parse_do_item(NecroParser* parser)
{
    ...
    case NECRO_LEX_LET:
    {
        ...
        NecroParseAstLocalPtr declarations = necro_try(NecroParseAstLocalPtr, necro_parse_declarations_list(parser));
        if (necro_parse_peek_token_type(parser) != NECRO_LEX_RIGHT_BRACE)
...
NecroResult(NecroParseAstLocalPtr) necro_parse_do(NecroParser* parser)
{
    ...
    // Statements
    NecroParseAstLocalPtr statement_list_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_list(parser, NECRO_LEX_SEMI_COLON, parse_do_item));
    if (statement_list_local_ptr == null_local_ptr)
    {
        necro_parse_restore(parser, snapshot);
        return ok_NecroParseAstLocalPtr(null_local_ptr);
...
NecroResult(NecroParseAstLocalPtr) necro_parse_expression_list(NecroParser* parser)
{
    ...
    // Expressions
    NecroParseAstLocalPtr statement_list_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_list(parser, NECRO_LEX_COMMA, necro_parse_expression));
...
NecroResult(NecroParseAstLocalPtr) necro_parse_pat_expression(NecroParser* parser)
{
    ...
    // aexpr
    NecroParseAstLocalPtr local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_application_expression(parser));
    if (local_ptr != null_local_ptr)
        return ok_NecroParseAstLocalPtr(local_ptr);
    necro_parse_restore(parser, snapshot);
...
NecroResult(NecroParseAstLocalPtr) necro_parse_expressions_for_pattern_expression(NecroParser* parser)
{
    ...
    NecroSourceLoc        source_loc           = necro_parse_peek_token(parser)->source_loc;
    NecroParserSnapshot   snapshot             = necro_parse_snapshot(parser);
    NecroParseAstLocalPtr expression_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_pat_expression(parser));
    if (expression_local_ptr == null_local_ptr)
    {
        necro_parse_restore(parser, snapshot);
        return ok_NecroParseAstLocalPtr(null_local_ptr);
    ...
    // Next Pat expression
    NecroParseAstLocalPtr next_expression_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_expressions_for_pattern_expression(parser));
...
NecroResult(NecroParseAstLocalPtr) necro_parse_pattern_expression(NecroParser* parser)
{
    ...
    // Expressions
    NecroParseAstLocalPtr expression_list = necro_try(NecroParseAstLocalPtr, necro_parse_expressions_for_pattern_expression(parser));
...
NecroResult(NecroParseAstLocalPtr) necro_parse_arithmetic_sequence(NecroParser* parser)
{
    ...
    NecroParseAstLocalPtr from = necro_try(NecroParseAstLocalPtr, necro_parse_expression(parser));
    ...
    {
        necro_parse_consume_token(parser); // consume ','
        then = necro_try(NecroParseAstLocalPtr, necro_parse_expression(parser));
    ...
    if (necro_parse_peek_token_type(parser) != NECRO_LEX_RIGHT_BRACKET)
    {
        to = necro_try(NecroParseAstLocalPtr, necro_parse_expression(parser));
...
NecroResult(NecroParseAstLocalPtr) necro_parse_case_alternative(NecroParser* parser)
{
    ...
    // body expr
    NecroParseAstLocalPtr body_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_expression(parser));
...
NecroResult(NecroParseAstLocalPtr) necro_parse_case(NecroParser* parser)
{
    ...
    // expr
    NecroParseAstLocalPtr case_expr_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_expression(parser));
    ...
    // alternatives
    NecroParseAstLocalPtr alternative_list_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_list(parser, NECRO_LEX_SEMI_COLON, necro_parse_case_alternative));
...
NecroResult(NecroParseAstLocalPtr) necro_parse_oppat(NecroParser* parser)
{
    ...
    // Left pat
    NecroParseAstLocalPtr left = necro_try(NecroParseAstLocalPtr, necro_parse_lpat(parser));
    if (left == null_local_ptr)
    {
        necro_parse_restore_state(parser, prev_state);
        necro_parse_restore(parser, snapshot);
    ...
    // Op
    NecroParseAstLocalPtr op = necro_try(NecroParseAstLocalPtr, necro_parse_qconop(parser, NECRO_VAR_VAR));
    ...
    // Right pat
    NecroParseAstLocalPtr right = necro_try(NecroParseAstLocalPtr, necro_parse_pat(parser));
    if (right == null_local_ptr)
    {
        necro_parse_restore_state(parser, prev_state);
        necro_parse_restore(parser, snapshot);
...
NecroResult(NecroParseAstLocalPtr) necro_parse_pat(NecroParser* parser)
{
    ...
    NECRO_PARSE_STATE     prev_state = necro_parse_enter_state(parser);
    NecroParserSnapshot   snapshot   = necro_parse_snapshot(parser);
    NecroParseAstLocalPtr ptr        = necro_try(NecroParseAstLocalPtr, necro_parse_oppat(parser));
    if (ptr == null_local_ptr)
    {
        ptr = necro_try(NecroParseAstLocalPtr, necro_parse_lpat(parser));
    }
...
NecroResult(NecroParseAstLocalPtr) necro_parse_lpat(NecroParser* parser)
{
    ...
    // gcon apat1 ... apatk
    {
        ptr = necro_try(NecroParseAstLocalPtr, necro_parse_gcon(parser, NECRO_CON_VAR));
        if (ptr != null_local_ptr)
        {
            NecroParseAstLocalPtr apats_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_apats(parser));
            if (apats_local_ptr != null_local_ptr)
    ...
    if (ptr == null_local_ptr)
    {
        ptr = necro_try(NecroParseAstLocalPtr, necro_parse_apat(parser));
    }
...
NecroResult(NecroParseAstLocalPtr) necro_parse_list_pat(NecroParser* parser)
{
    ...
    // Pats
    NecroParseAstLocalPtr list_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_list(parser, NECRO_LEX_COMMA, necro_parse_pat));
...
NecroParseAstLocalPtr              necro_parse_simpletype(NecroParser* parser);
...
NecroResult(NecroParseAstLocalPtr) necro_parse_top_level_data_declaration(NecroParser* parser)
{
    ...
    // simpletype
    NecroParseAstLocalPtr simpletype = necro_parse_simpletype(parser);
    ...
    // constructor_list
    NecroParseAstLocalPtr constructor_list = necro_try(NecroParseAstLocalPtr, necro_parse_list(parser, NECRO_LEX_PIPE, necro_parse_constr));
...
NecroResult(NecroParseAstLocalPtr) necro_parse_atype_list(NecroParser* parser)
{
    ...
    // tyvar
    NecroSourceLoc        source_loc = necro_parse_peek_token(parser)->source_loc;
    NecroParseAstLocalPtr atype      = necro_try(NecroParseAstLocalPtr, necro_parse_atype(parser, NECRO_VAR_TYPE_FREE_VAR));
    if (atype == null_local_ptr)
        return ok_NecroParseAstLocalPtr(null_local_ptr);

    // next
    NecroParseAstLocalPtr next = necro_try(NecroParseAstLocalPtr, necro_parse_atype_list(parser));
...
NecroResult(NecroParseAstLocalPtr) necro_parse_constr(NecroParser* parser)
{
    ...
    // atype list
    NecroParseAstLocalPtr atype_list = necro_try(NecroParseAstLocalPtr, necro_parse_atype_list(parser));
...
NecroParseAstLocalPtr necro_parse_tyvar_list(NecroParser* parser)
{
    if (necro_parse_peek_token_type(parser) == NECRO_LEX_END_OF_STREAM)
        return null_local_ptr;
    NecroSourceLoc source_loc = necro_parse_peek_token(parser)->source_loc;
    ...
    // tyvar
    NecroParseAstLocalPtr tyvar = necro_parse_tyvar(parser, NECRO_VAR_TYPE_VAR_DECLARATION);
    if (tyvar == null_local_ptr)
        return null_local_ptr;

    // next
    NecroParseAstLocalPtr next = necro_parse_tyvar_list(parser);
...
NecroParseAstLocalPtr necro_parse_simpletype(NecroParser* parser)
{
    ...
    // tyvar list
    NecroParseAstLocalPtr tyvars = necro_parse_tyvar_list(parser);
...
NecroResult(NecroParseAstLocalPtr) necro_parse_type(NecroParser* parser)
{
    ...
    NecroSourceLoc        source_loc = necro_parse_peek_token(parser)->source_loc;
    NecroParserSnapshot   snapshot   = necro_parse_snapshot(parser);
    NecroParseAstLocalPtr ty_ptr      = necro_try(NecroParseAstLocalPtr, necro_parse_btype(parser));
    ...
    {
        necro_parse_consume_token(parser);
        NecroParseAstLocalPtr next_after_arrow_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_type(parser));
...
NecroResult(NecroParseAstLocalPtr) necro_parse_btype(NecroParser* parser)
{
    ...
    NecroParserSnapshot   snapshot = necro_parse_snapshot(parser);
    NecroParseAstLocalPtr atype    = necro_try(NecroParseAstLocalPtr, necro_parse_atype(parser, NECRO_VAR_TYPE_FREE_VAR));
    NecroParseAstLocalPtr btype    = necro_try(NecroParseAstLocalPtr, necro_parse_btype_go(parser, atype));
...
NecroResult(NecroParseAstLocalPtr) necro_parse_atype(NecroParser* parser, NECRO_VAR_TYPE tyvar_var_type)
{
    ...
    if (ptr == null_local_ptr)
    {
        ptr = necro_try(NecroParseAstLocalPtr, necro_parse_tuple_type(parser));
    }

    // [type]
    if (ptr == null_local_ptr)
    {
        ptr = necro_try(NecroParseAstLocalPtr, necro_parse_list_type(parser));
    }
    ...
    if (ptr == null_local_ptr)
    {
        ...
        {
            necro_parse_consume_token(parser);
            ptr = necro_try(NecroParseAstLocalPtr, necro_parse_type(parser));
            if (necro_parse_peek_token_type(parser) != NECRO_LEX_RIGHT_PAREN)
            {
                necro_parse_restore(parser, snapshot);
                ptr = null_local_ptr;
...
NecroResult(NecroParseAstLocalPtr) necro_parse_tuple_type(NecroParser* parser)
{
    ...
    // types
    NecroParseAstLocalPtr list_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_list(parser, NECRO_LEX_COMMA, necro_parse_type));
    if (list_local_ptr == null_local_ptr)
    {
        necro_parse_restore(parser, snapshot);
        return ok_NecroParseAstLocalPtr(null_local_ptr);
...
NecroResult(NecroParseAstLocalPtr) necro_parse_context(NecroParser* parser)
{
    ...
    {
        necro_parse_consume_token(parser);
        NecroParseAstLocalPtr class_list = necro_try(NecroParseAstLocalPtr, necro_parse_list(parser, NECRO_LEX_COMMA, necro_parse_simple_class));
...
NecroResult(NecroParseAstLocalPtr) necro_parse_type_signature(NecroParser* parser, NECRO_SIG_TYPE sig_type)
{
    ...
    NecroParseAstLocalPtr context          = necro_try(NecroParseAstLocalPtr, necro_parse_context(parser));
    ...
    // type
    NecroParseAstLocalPtr type = necro_try(NecroParseAstLocalPtr, necro_parse_type(parser));
    ...
    // Finish
    NecroParseAstLocalPtr local_ptr = necro_parse_ast_create_type_signature(&parser->ast.arena, source_loc, necro_parse_peek_token(parser)->end_loc, var, context, type, sig_type);
    return ok_NecroParseAstLocalPtr(local_ptr);
}
...
NecroResult(NecroParseAstLocalPtr) necro_parse_class_declaration(NecroParser* parser)
{
    ...
    if (declaration_local_ptr == null_local_ptr)
    {
        declaration_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_type_signature(parser, NECRO_SIG_TYPE_CLASS));
    }
...
NecroResult(NecroParseAstLocalPtr) necro_parse_class_declarations_list(NecroParser* parser)
{
    ...
    NecroParseAstLocalPtr declaration_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_class_declaration(parser));
    ...
    if (necro_parse_peek_token_type(parser) == NECRO_LEX_SEMI_COLON)
    {
        ...
        next_decl = necro_try(NecroParseAstLocalPtr, necro_parse_class_declarations_list(parser));
    }
...
NecroResult(NecroParseAstLocalPtr) necro_parse_type_class_declaration(NecroParser* parser)
{
    ...
    if (necro_parse_peek_token_type(parser) == NECRO_LEX_WHERE)
    {
        ...
        {
            necro_parse_consume_token(parser);
            declarations = necro_try(NecroParseAstLocalPtr, necro_parse_class_declarations_list(parser));
            if (necro_parse_peek_token_type(parser) != NECRO_LEX_RIGHT_BRACE)
...
NecroResult(NecroParseAstLocalPtr) necro_parse_instance_declaration_list(NecroParser* parser)
{
    ...
    // Declaration
    NecroParserSnapshot   snapshot              = necro_parse_snapshot(parser);
    NecroParseAstLocalPtr declaration_local_ptr = necro_try(NecroParseAstLocalPtr, necro_parse_instance_declaration(parser));
    if (declaration_local_ptr == null_local_ptr)
    {
        necro_parse_restore(parser, snapshot);
        return ok_NecroParseAstLocalPtr(null_local_ptr);
    ...
    {
        necro_parse_consume_token(parser);
        next_decl = necro_try(NecroParseAstLocalPtr, necro_parse_instance_declaration_list(parser));
    }
...
NecroParseAstLocalPtr necro_parse_inst_constr(NecroParser* parser)
{
    if (necro_parse_peek_token_type(parser) == NECRO_LEX_END_OF_STREAM)
        return null_local_ptr;
    ...
    // tyvar list
    NecroParseAstLocalPtr ty_var_list = necro_parse_tyvar_list(parser);
    if (necro_parse_peek_token_type(parser) == NECRO_LEX_END_OF_STREAM)
    {
        necro_parse_restore(parser, snapshot);
        return null_local_ptr;
...
NecroParseAstLocalPtr necro_parse_inst(NecroParser* parser)
{
    if (necro_parse_peek_token_type(parser) == NECRO_LEX_END_OF_STREAM)
        return null_local_ptr;
    ...
    ptr = necro_parse_inst_constr(parser);
    if (ptr != null_local_ptr)
        return ptr;
...
NecroResult(NecroParseAstLocalPtr) necro_parse_type_class_instance(NecroParser* parser)
{
    ...
    // inst
    NecroParseAstLocalPtr inst = necro_parse_inst(parser);
    ...
    if (necro_parse_peek_token_type(parser) == NECRO_LEX_WHERE)
    {
        ...
        {
            necro_parse_consume_token(parser);
            declarations = necro_try(NecroParseAstLocalPtr, necro_parse_instance_declaration_list(parser));
            if (necro_parse_peek_token_type(parser) != NECRO_LEX_RIGHT_BRACE)
