...
void necro_print_node_value(NecroNodeProgram* program, NecroNodeAST* ast, NECRO_SHOULD_PRINT_VALUE_TYPE should_print_value_type)
{
    ...
    {
        printf(" (");
        necro_node_print_node_type_go(program->intern, ast->necro_node_type, false);
        printf(")");
    }
}
...
void necro_node_print_load(NecroNodeProgram* program, NecroNodeAST* ast, size_t depth)
{
    assert(ast->type == NECRO_NODE_LOAD);
    printf("%%%s = load ", necro_intern_get_string(program->intern, ast->load.dest_value->value.reg_name.symbol));
    switch (ast->load.load_type)
    {
    case NECRO_LOAD_TAG:
        necro_print_node_value(program, ast->load.source_ptr, NECRO_PRINT_VALUE_TYPE);
        printf(" (tag)");
        return;
    case NECRO_LOAD_PTR:
        necro_print_node_value(program, ast->load.source_ptr, NECRO_PRINT_VALUE_TYPE);
...
void necro_node_print_gep(NecroNodeProgram* program, NecroNodeAST* ast, size_t depth)
{
    assert(ast->type == NECRO_NODE_GEP);
...
void necro_node_print_ast_go(NecroNodeProgram* program, NecroNodeAST* ast, size_t depth)
{
    ...
    case NECRO_NODE_LOAD:
        print_white_space(depth);
        necro_node_print_load(program, ast, depth);
        return;
