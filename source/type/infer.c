...
//     NecroType* con_type_2 = necro_create_type_con(infer, con_type->con.con, NULL, con_type->con.arity);
//     con_type_2->source_loc = con_type->source_loc;
//     return con_type_2;
// }
...
NecroType* necro_ast_to_type_sig_go(NecroInfer* infer, NecroNode* ast)
{
    ...
    case NECRO_AST_CONID:
    {
        ...
        // NecroType* con_type     = necro_create_type_con(infer, env_con_type->con.con, NULL, necro_kind_arity(env_con_type->kind));
    ...
    case NECRO_AST_CONSTRUCTOR:
    {
        ...
        // Applied to too many args error
        if (arity > con_type->con.arity)
            return necro_infer_ast_error(infer, con_type, ast, "%s is applied to too many arguments. Expected arity: %d, but found arity %d", necro_intern_get_string(infer->intern, con_type->con.con.symbol), con_type->con.arity, arity);
        return con_type;
    }
...
NecroType* necro_infer_type_sig(NecroInfer* infer, NecroNode* ast)
{
    ...
    if (necro_is_infer_error(infer)) return NULL;
    necro_check_type_sanity(infer, type_sig, type_sig, "While constructing user supplied type signature: ");
    if (necro_is_infer_error(infer)) return NULL;
...
NecroType* necro_infer_simple_type(NecroInfer* infer, NecroNode* ast)
{
    ...
    NecroType* type = necro_create_type_con(infer, (NecroCon) { .symbol = ast->simple_type.type_con->conid.symbol, .id = ast->simple_type.type_con->conid.id.id }, NULL, necro_count_ty_vars(ast->simple_type.type_var_list));
    ...
    if (necro_is_infer_error(infer)) return NULL;
    necro_check_type_sanity(infer, type, type, "While compiling data declaration: ");
    if (necro_is_infer_error(infer)) return NULL;
...
NecroType* necro_infer_var(NecroInfer* infer, NecroNode* ast)
{
    ...
    if (var_type == NULL)
    {
        var_type                = necro_new_name(infer, ast->source_loc);
        var_type->source_loc    = ast->source_loc;
        var_type->var.scope     = infer->symtable->data[ast->variable.id.id].scope;
...
NecroType* necro_infer_apat(NecroInfer* infer, NecroNode* ast)
{
    ...
    case NECRO_AST_BIN_OP_SYM:
    {
        ...
        if (necro_is_infer_error(infer)) return NULL;
        necro_check_type_sanity(infer, data_type, constructor_type, "While inferring the type of a bin-op pattern");
        return data_type;
    }
    ...
    case NECRO_AST_CONSTRUCTOR:
    {
        ...
        necro_check_type_sanity(infer, pattern_type_head, pattern_type_head, "While inferring the type of a constructor pattern");
        if (necro_is_infer_error(infer)) return NULL;
        ...
        necro_check_type_sanity(infer, data_type, data_type, "While inferring the type of a constructor pattern: ");
        if (necro_is_infer_error(infer)) return NULL;
        return data_type;
    }
...
NecroType* necro_infer_declaration(NecroInfer* infer, NecroNode* ast)
{
    ...
    while (current_decl != NULL)
    {
        ...
        case NECRO_AST_SIMPLE_ASSIGNMENT:
            if (necro_symtable_get(infer->symtable, current_decl->declaration.declaration_impl->simple_assignment.id)->type == NULL)
            {
                ...
                new_name->var.scope  = infer->symtable->data[current_decl->declaration.declaration_impl->simple_assignment.id.id].scope;
        ...
        case NECRO_AST_APATS_ASSIGNMENT:
            if (necro_symtable_get(infer->symtable, current_decl->declaration.declaration_impl->apats_assignment.id)->type == NULL)
            {
                NecroType* new_name  = necro_new_name(infer, ast->source_loc);
                new_name->source_loc = current_decl->declaration.declaration_impl->source_loc;
                new_name->var.scope  = infer->symtable->data[current_decl->declaration.declaration_impl->apats_assignment.id.id].scope;
    ...
    while (current_decl != NULL)
    {
        ...
        case NECRO_AST_SIMPLE_ASSIGNMENT:
            ...
            {
                NecroVar   var        = (NecroVar) { .id = current_decl->declaration.declaration_impl->simple_assignment.id, .symbol = NULL_SYMBOL};
...
NecroType* necro_infer_top_declaration(NecroInfer* infer, NecroNode* ast)
{
    ...
    while (current_decl != NULL)
    {
        ...
        case NECRO_AST_SIMPLE_ASSIGNMENT:
            if (necro_symtable_get(infer->symtable, current_decl->top_declaration.declaration->simple_assignment.id)->type == NULL)
            {
                ...
                new_name->var.scope  = infer->symtable->data[current_decl->top_declaration.declaration->simple_assignment.id.id].scope;
    ...
    while (current_decl != NULL)
    {
        ...
        case NECRO_AST_SIMPLE_ASSIGNMENT:
            ...
            {
                NecroVar   var        = (NecroVar) { .id = current_decl->top_declaration.declaration->simple_assignment.id, .symbol = NULL_SYMBOL };
                NecroType* proxy_type = infer->symtable->data[var.id.id].type;
                infer->symtable->data[var.id.id].type = necro_gen(infer, proxy_type, infer->symtable->data[var.id.id].scope->parent);
            }
            break;
        case NECRO_AST_APATS_ASSIGNMENT:
            ...
            {
                NecroVar   var        = (NecroVar) { .id = current_decl->top_declaration.declaration->apats_assignment.id, .symbol = NULL_SYMBOL };
                NecroType* proxy_type = infer->symtable->data[var.id.id].type;
                infer->symtable->data[var.id.id].type = necro_gen(infer, proxy_type, infer->symtable->data[var.id.id].scope->parent);
            }
            break;
...
NecroType* necro_infer(NecroInfer* infer, NecroNode* ast)
{
    ...
    necro_check_type_sanity(infer, result, result, NULL);
    if (infer->error.return_code != NECRO_SUCCESS)
        return NULL;
    return result;
}
