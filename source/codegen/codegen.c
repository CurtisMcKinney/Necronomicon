...
NecroCodeGen necro_create_codegen(NecroInfer* infer, NecroIntern* intern, NecroSymTable* symtable, NecroRuntime* runtime, const char* module_name)
{
    ...
    // const char* target_triple = LLVMGetTarget(mod);
    // printf("target: %s\n", target_triple);
    // // printf("default: %s\n", default_triple);
    // // const char* target_desc = LLVMGetTargetDescription();
    ...
    for (size_t r = 0; r < matrix->rows; ++r)
    {
        ...
        if (row_head == NULL) // WILDCARD....why Chad? Why!?!??!
        {
            for (size_t c_d = 0; c_d < column_diff; ++c_d)
            {
                ...
                specialized_matrix.paths[c_d]           = NULL;
            }
...
NecroPatternMatrix necro_create_pattern_matrix_from_case(NecroCodeGen* codegen, NecroCoreAST_Expression* ast, NecroPatternPath* case_path)
{
    ...
    for (size_t c = 0; c < columns; ++c)
    {
        matrix.paths[c] = necro_create_pattern_path(codegen, case_path, case_path->value, c);
    }
    return matrix;
}
...
void necro_codegen_decision_tree(NecroCodeGen* codegen, NecroDecisionTree* tree, LLVMBasicBlockRef current_block, LLVMBasicBlockRef term_case_block, LLVMBasicBlockRef error_block, LLVMValueRef result_phi, NecroNodePrototype* outer)
{
    if (necro_is_codegen_error(codegen)) return;
    assert(codegen != NULL);
    assert(tree != NULL);
    ...
    case NECRO_DECISION_TREE_LEAF:
    {
        ...
        LLVMBasicBlockRef current_block = LLVMGetLastBasicBlock(codegen->current_func);
    ...
    case NECRO_DECISION_TREE_SWITCH:
    {
        ...
        {
            // tree->tree_switch.path->parent->value
            ...
            tree->tree_switch.path->value  = LLVMBuildLoad(codegen->builder, offset_parent_ptr, "path_slot_val");
        }
        ...
        for (size_t i = 0; i < tree->tree_switch.num_cases; ++i)
        {
            ...
            LLVMBasicBlockRef case_block = LLVMAppendBasicBlockInContext(codegen->context, codegen->current_func, case_name);
            ...
            necro_codegen_decision_tree(codegen, tree->tree_switch.cases[i], case_block, term_case_block, error_block, result_phi, outer);
        }
        break;
    }
    }
...
LLVMValueRef necro_codegen_case(NecroCodeGen* codegen, NecroCoreAST_Expression* ast, NecroNodePrototype* outer)
{
    ...
    assert(ast->case_expr.alts != NULL);
    assert(ast->case_expr.type != NULL);
    // TODO: accrue list of (var, LLVMValue, block) tuples!?!?!? How to handle bound variables in patterns!?
    ...
    NecroPatternPath*  case_path       = necro_create_pattern_path(codegen, NULL, case_expr_value, 0);
    ...
    LLVMBasicBlockRef  current_block   = LLVMGetLastBasicBlock(codegen->current_func);
    LLVMBasicBlockRef  err_block       = LLVMAppendBasicBlock(codegen->current_func, "case_error");
    LLVMPositionBuilderAtEnd(codegen->builder, err_block);
    LLVMValueRef       trap_call       = necro_build_call(codegen, codegen->llvm_intrinsics.trap, NULL, 0, "");
    LLVMSetInstructionCallConv(trap_call, LLVMCCallConv);
    LLVMBuildUnreachable(codegen->builder);
    LLVMBasicBlockRef  term_case_block = LLVMAppendBasicBlock(codegen->current_func, "case_end");
    ...
    LLVMPositionBuilderAtEnd(codegen->builder, current_block);
    // TODO: Initial block
    necro_codegen_decision_tree(codegen, tree, NULL, term_case_block, err_block, case_result, outer);    if (necro_is_codegen_error(codegen)) return NULL;
    LLVMPositionBuilderAtEnd(codegen->builder, term_case_block);
    return case_result;
}
...
LLVMValueRef necro_calculate_node_call_bind(NecroCodeGen* codegen, NecroCoreAST_Expression* ast, NecroNodePrototype* outer)
{
    ...
    if (node_prototype->type == NECRO_NODE_STATEFUL)
    {
        ...
        if (arg_count == 1)
        {
            ...
            // LLVMValueRef print_val  = necro_build_call(codegen, codegen->runtime->functions.necro_print, (LLVMValueRef[]) { cast_inull }, 1, "");
...
void necro_codegen_main(NecroCodeGen* codegen, NecroCoreAST_Expression* ast)
{
    ...
    // Init roots
    LLVMValueRef init_roots = necro_build_call(codegen, codegen->runtime->functions.necro_initialize_root_set, (LLVMValueRef[]) { LLVMConstInt(LLVMInt32TypeInContext(codegen->context), global_count, false) }, 1, "");
    ...
    while (top != NULL)
    {
        if (top->list.expr != NULL && top->list.expr->expr_type == NECRO_CORE_EXPR_BIND)
        {
            ...
            // LLVMValueRef print_instr    = necro_build_call(codegen, codegen->runtime->functions.necro_print, (LLVMValueRef[]) { data_size }, 1, "");
            // LLVMSetInstructionCallConv(print_instr, LLVMCCallConv);

            // // TODO: Two simple loads and stores instead!?
            // // LLVMValueRef cpy_ref     = necro_build_call(codegen, codegen->llvm_intrinsics.mem_cpy, (LLVMValueRef[]) { cast_glob, cast_node, data_size, is_volatile }, 4, "");
            ...
            // LLVMValueRef cpy_ref     = necro_build_call(codegen, codegen->llvm_intrinsics.mem_cpy, (LLVMValueRef[]) { cast_glob, cast_node, n_data_size, is_volatile }, 4, "");
            ...
            // Set root
            LLVMValueRef set_root  = necro_build_call(codegen, codegen->runtime->functions.necro_set_root, (LLVMValueRef[]) { cast_glob, LLVMConstInt(LLVMInt32TypeInContext(codegen->context), global_count, false) }, 2, "");
    ...
    // GC
    LLVMValueRef gc_instr = necro_build_call(codegen, codegen->runtime->functions.necro_collect, NULL, 0, "");
    LLVMSetInstructionCallConv(gc_instr, LLVMCCallConv);
    // Looping logic...
    LLVMValueRef sleep_instr = necro_build_call(codegen, codegen->runtime->functions.necro_sleep, (LLVMValueRef[]) { LLVMConstInt(LLVMInt32TypeInContext(codegen->context), 11, false) }, 1, "");
...
LLVMValueRef necro_alloc_codegen(NecroCodeGen* codegen, LLVMTypeRef type, uint8_t a_slots_used)
{
    LLVMValueRef data_size  = LLVMSizeOf(type);
    LLVMValueRef slots_used = LLVMConstInt(LLVMInt8TypeInContext(codegen->context), a_slots_used, false);
    LLVMValueRef void_ptr   = necro_build_call(codegen, codegen->runtime->functions.necro_alloc, (LLVMValueRef[]) { data_size, slots_used }, 2, "void_ptr");
