...
bool necro_ambig_occurs(NecroInfer* infer, NecroCon name, NecroType* type)
{
    ...
    default:              necro_infer_ast_error(infer, type, NULL, "Compiler bug: Unrecognized type: %d.", type->type);
    }
    return false;
}
...
void necro_print_type_class_member(NecroTypeClassMember* member, NecroInfer* infer, NecroIntern* intern, size_t white_count)
{
    ...
    printf("%s :: ", member->member_varid.symbol->str);
...
void necro_print_type_class_context(NecroTypeClassContext* context, NecroInfer* infer, NecroIntern* intern, size_t white_count)
{
    ...
    if (context->next != NULL)
        printf("%s %s,", context->type_class_name.symbol->str, context->type_var.symbol->str);
    else
        printf("%s %s", context->type_class_name.symbol->str, context->type_var.symbol->str);
...
void necro_print_type_class(NecroTypeClass* type_class, NecroInfer* infer, NecroIntern* intern, size_t white_count)
{
    ...
    printf("class %s %s\n", type_class->type_class_name.symbol->str, type_class->type_var.symbol->str);
    print_white_space(white_count);
    printf("{\n");
    print_white_space(white_count + 4);
    printf("type:    ");
...
void necro_print_dictionary_prototype(NecroDictionaryPrototype* prototype, NecroInfer* infer, NecroIntern* intern, size_t white_count)
{
    ...
    print_white_space(white_count + 4);
    printf("%s :: ", prototype->prototype_varid.symbol->str);
    necro_print_type_sig(member_type, intern);
...
void necro_print_instance(NecroTypeClassInstance* instance, NecroInfer* infer, NecroIntern* intern, size_t white_count)
{
    if (instance == NULL)
        return;
    print_white_space(white_count);
    printf("instance %s %s\n", instance->type_class_name.symbol->str, instance->data_type_name.symbol->str);
    print_white_space(white_count);
    printf("{\n");
    print_white_space(white_count + 4);
    printf("context: (");
...
NecroSymbol necro_create_dictionary_arg_name(NecroIntern* intern, NecroSymbol type_class_name, NecroID id)
{
    ...
    NecroSymbol id_string           = necro_intern_string(intern, necro_id_as_character_string(intern, (NecroVar) { .id = id, .symbol = NULL }));
...
NecroAst* retrieveDictionaryFromContext(NecroInfer* infer, NecroTypeClassDictionaryContext* dictionary_context, NecroTypeClassContext* dictionary_to_retrieve, NecroVar type_var)
{
...
NecroAst* necro_resolve_context_to_dictionary(NecroInfer* infer, NecroTypeClassContext* context, NecroTypeClassDictionaryContext* dictionary_context)
{
    ...
    case NECRO_TYPE_VAR:
    {
        ...
        if (var_type->var.is_rigid)
        {
            ...
            NecroAst* d_var = retrieveDictionaryFromContext(infer, dictionary_context, context, necro_con_to_var(var_type->var.context->type_var));
            assert(d_var != NULL);
            return d_var;
        }
        else
        {
            ...
            else
            {
                ...
                NecroAst* d_var = retrieveDictionaryFromContext(infer, dictionary_context, context, necro_find(var_type->var.gen_bound)->var.var);
                assert(d_var != NULL);
                return d_var;
            }
        }
...
NecroAst* necro_resolve_method(NecroInfer* infer, NecroTypeClass* method_type_class, NecroTypeClassContext* context, NecroAst* ast, NecroTypeClassDictionaryContext* dictionary_context)
{
    ...
    case NECRO_TYPE_CON:
    {
        ...
        while (prototype != NULL)
        {
            ...
            {
                NecroAst* m_var                         = necro_ast_create_var(&infer->arena, infer->intern, prototype->prototype_varid.symbol->str, NECRO_VAR_VAR);
...
NecroTypeClassDictionaryContext* necro_create_type_class_dictionary_context(NecroPagedArena* arena, NecroCon type_class_name, NecroCon type_var_name, NecroAst* dictionary_arg_ast, NecroTypeClassDictionaryContext* next)
{
...
void necro_type_class_translate_go(NecroTypeClassDictionaryContext* dictionary_context, NecroInfer* infer, NecroAst* ast)
{
    ...
    case NECRO_AST_VARIABLE:
        ...
        case NECRO_VAR_VAR:
        {
            ...
            {
                if (necro_is_infer_error(infer)) return;
                NecroAst* var_ast = necro_ast_create_var(&infer->arena, infer->intern, ast->variable.symbol->str, NECRO_VAR_VAR);
                *var_ast = *ast;
                NecroTypeClassContext* inst_context = var_ast->variable.inst_context;
                assert(inst_context != NULL);
                NecroAst* m_var = necro_resolve_method(infer, var_info->method_type_class, inst_context, var_ast, dictionary_context);
                if (necro_is_infer_error(infer)) return;
                assert(m_var != NULL);
                m_var->scope   = ast->scope;
                necro_rename_var_pass(infer->renamer, &infer->arena, m_var);
    ...
    case NECRO_AST_BIN_OP:
    {
        ...
        NecroAst* op_ast          = necro_ast_create_var(&infer->arena, infer->intern, ast->bin_op.symbol->str, NECRO_VAR_VAR);
    ...
    {
        assert(ast->necro_type != NULL);
        NecroAst* op_ast          = necro_ast_create_var(&infer->arena, infer->intern, ast->op_left_section.symbol->str, NECRO_VAR_VAR);
    ...
    case NECRO_AST_OP_RIGHT_SECTION:
    {
        ...
        NecroAst* op_ast          = necro_ast_create_var(&infer->arena, infer->intern, ast->op_right_section.symbol->str, NECRO_VAR_VAR);
        ...
        while (group_list != NULL)
        {
            ...
            while (declaration_group != NULL)
            {
                ...
                while (curr_context != NULL)
                {
                    ...
                    NecroType*      curr_type_class_var  = necro_symtable_get(infer->symtable, curr_type_class->type_var.id)->type;
                    NecroType*      var_type             = necro_symtable_get(infer->symtable, curr_context->type_var.id)->type;
                    necro_kind_unify(infer, var_type->type_kind, curr_type_class_var->type_kind, method_ast->scope, type_sig, "While inferring the type of a method type signature");
                    if (necro_is_infer_error(infer)) return;
    ...
    if (instance->ast->type_class_instance.declarations != NULL)
    {
        ...
        while (group_list != NULL)
        {
            ...
            while (declaration_group != NULL)
            {
                ...
                while (members != NULL)
                {
                    if (members->member_varid.symbol == member_symbol)
                    {
    ...
    // Create Dictionary Instance
    {
        ...
        while (members != NULL)
        {
            ...
            while (dictionary_prototype != NULL)
            {
                ...
                {
                    NecroAst* mvar = necro_ast_create_var(&infer->arena, infer->intern, dictionary_prototype->prototype_varid.symbol->str, NECRO_VAR_VAR);
                    ...
                    while (inst_context != NULL)
                    {
                        NecroType* inst_context_type_var_type = necro_find(infer->env.data[inst_context->type_var.id.id]);
                        NecroCon   inst_context_type_var      = inst_context_type_var_type->var.context->type_var;
                        NecroAst* dvar = necro_ast_create_var(&infer->arena, infer->intern, necro_create_dictionary_arg_name(infer->intern, inst_context->type_class_name.symbol, inst_context->type_var.id)->str, NECRO_VAR_VAR);
