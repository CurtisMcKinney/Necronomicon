...
extern DLLEXPORT uint8_t* necro_runtime_alloc(size_t size)
{
    ...
    if (size == 0)
        return NULL;
    // size += 8; // PADDING?!?!?! This will crash without padding, suggesting that our allocation sizes are getting off somewhere, or an errant writeArray is happening in NecroLang somewhere!?!?!?!
