...
void necro_core_to_machine_1_data_con(NecroMachineProgram* program, NecroCoreAST_DataCon* con, NecroMachineAST* struct_type, size_t max_arg_count, size_t con_number)
{
    ...
    for (size_t c = 0; c < 2; ++c)
    {
        ...
        NecroMachineAST*   data_ptr        = necro_build_nalloc(program, mk_fn_def, struct_type->necro_machine_type, arg_count + 1, c == 1);
...
NecroSlot necro_add_member(NecroMachineProgram* program, NecroMachineDef* machine_def, NecroMachineType* new_member, size_t data_id)
{
    assert(program != NULL);
    ...
    NecroSlot slot = (NecroSlot) { .necro_machine_type = new_member, .slot_num = machine_def->num_members, .machine_def = machine_def, .data_id = data_id};
...
void necro_calculate_statefulness(NecroMachineProgram* program, NecroMachineAST* ast)
{
    ...
    if (ast->machine_def.is_recursive && ast->machine_def.num_arg_names == 0 && ast->machine_def.num_members == 0 && ast->machine_def.outer == NULL)
        ast->machine_def.state_type = NECRO_STATE_POINTWISE;
...
void necro_core_to_machine_2_bind(NecroMachineProgram* program, NecroCoreAST_Expression* core_ast, NecroMachineAST* outer)
{
    ...
    // if (machine_def->machine_def.state_type < NECRO_STATE_STATEFUL || (is_machine_fn && !machine_def->machine_def.is_recursive))
    // Don't make recursive non-stateful machines NECRO_STATE_STATEFUL
...
void necro_core_to_machine_2_app(NecroMachineProgram* program, NecroCoreAST_Expression* core_ast, NecroMachineAST* outer)
{
    ...
    if (!is_dynamic)
    {
        ...
        core_ast->app.persistent_slot = necro_add_member(program, &outer->machine_def, fn_def->necro_machine_type, data_id).slot_num;
    }
    else
    {
        fn_def->machine_def.is_recursive = true;
        core_ast->app.persistent_slot    = necro_add_member(program, &outer->machine_def, necro_create_machine_ptr_type(&program->arena, fn_def->necro_machine_type), NECRO_NULL_DATA_ID).slot_num;
...
void necro_core_to_machine_2_var(NecroMachineProgram* program, NecroCoreAST_Expression* core_ast, NecroMachineAST* outer)
{
    ...
    size_t data_id = necro_create_data_info(program, necro_symtable_get(program->symtable, core_ast->var.id)->type);
    ...
    NecroSlot slot = necro_is_unboxed_type(program, machine_ast->machine_def.value_type)
        ? necro_add_member(program, &outer->machine_def, machine_ast->machine_def.value_type, data_id)
        : necro_add_member(program, &outer->machine_def, necro_create_machine_ptr_type(&program->arena, machine_ast->machine_def.value_type), data_id);
...
NecroMachineAST* necro_core_to_machine_3_var(NecroMachineProgram* program, NecroCoreAST_Expression* core_ast, NecroMachineAST* outer)
{
    ...
    if (info->is_constructor)
    {
        if (outer->machine_def.state_type == NECRO_STATE_CONSTANT)
            return necro_build_call(program, outer->machine_def.update_fn, info->const_necro_machine_ast, NULL, 0, "const_con");
        else
...
void necro_construct_main(NecroMachineProgram* program)
{
    ...
    // Create world
    NecroMachineAST* world_value = necro_build_nalloc(program, necro_main_fn, program->world_type, 1, true);
    ...
    for (size_t i = 0; i < program->machine_defs.length; ++i)
    {
        if (program->machine_defs.data[i]->machine_def.state_type > NECRO_STATE_CONSTANT || program->machine_defs.data[i]->machine_def.num_arg_names != 0)
            continue;
        NecroMachineAST* const_value = necro_build_call(program, necro_main_fn, program->machine_defs.data[i]->machine_def.update_fn->fn_def.fn_value, NULL, 0, "const_result");
    ...
    for (size_t i = 0; i < program->machine_defs.length; ++i)
    {
        if (program->machine_defs.data[i]->machine_def.state_type == NECRO_STATE_STATEFUL && program->machine_defs.data[i]->machine_def.num_arg_names == 0)
        {
            ...
            NecroMachineAST* result = necro_build_call(program, necro_main_fn, program->machine_defs.data[i]->machine_def.update_fn->fn_def.fn_value, (NecroMachineAST*[]) { state }, 1, "stateful_result");
        ...
        {
            NecroMachineAST* result = necro_build_call(program, necro_main_fn, program->machine_defs.data[i]->machine_def.update_fn->fn_def.fn_value, NULL, 0, "pointwise_result");
            necro_build_store_into_ptr(program, necro_main_fn, result, program->machine_defs.data[i]->machine_def.global_value);
        }
    }
